...
class grid_usuarios_xls
{
   ...
   function inicializa_vars()
   {
      ...
      $this->arquivo   .= ".xls";
      $this->tit_doc    = "grid_usuarios.xls";
   ...
   function grava_arquivo()
   {
      ...
      { 
          ...
          if ($Cada_col == "fbid" && (!isset($this->NM_cmp_hidden[$Cada_col]) || $this->NM_cmp_hidden[$Cada_col] != "off"))
          {
             if (!NM_is_utf8($SC_Label))
              {
                  $SC_Label = mb_convert_encoding($SC_Label, "UTF-8", $_SESSION['scriptcase']['charset']);
      ...
      while (!$rs->EOF)
      {
         ...
         $this->fbid = $rs->fields[6] ;  
         $this->nombrecompleto = $rs->fields[7] ;  
   ...
   function NM_export_pais()
   {
         ...
         if (!NM_is_utf8($this->pais))
         {
             $this->pais = mb_convert_encoding($this->pais, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->Nm_ActiveSheet->getStyle($this->calc_cell($this->xls_col) . $this->xls_row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
         $this->Nm_ActiveSheet->setCellValue($this->calc_cell($this->xls_col) . $this->xls_row, $this->pais);
         $this->xls_col++;
   }
   //----- creado
   ...
   //----- fbid
   function NM_export_fbid()
   {
         $this->fbid = html_entity_decode($this->fbid, ENT_COMPAT, $_SESSION['scriptcase']['charset']);
         $this->fbid = strip_tags($this->fbid);
         if (!NM_is_utf8($this->fbid))
         {
             $this->fbid = mb_convert_encoding($this->fbid, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->Nm_ActiveSheet->getStyle($this->calc_cell($this->xls_col) . $this->xls_row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
         $this->Nm_ActiveSheet->setCellValue($this->calc_cell($this->xls_col) . $this->xls_row, $this->fbid);
         $this->xls_col++;
   }
   //----- nombrecompleto
   function NM_export_nombrecompleto()
   {
         ...
         if (!NM_is_utf8($this->nombrecompleto))
         {
             $this->nombrecompleto = mb_convert_encoding($this->nombrecompleto, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->Nm_ActiveSheet->getStyle($this->calc_cell($this->xls_col) . $this->xls_row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
         $this->Nm_ActiveSheet->setCellValue($this->calc_cell($this->xls_col) . $this->xls_row, $this->nombrecompleto);
         $this->xls_col++;
   }
...
<HEAD>
 <TITLE><?php echo $this->Ini->Nm_lang['lang_othr_grid_titl'] ?> - usuarios :: Excel</TITLE>
