<form name="F2" method=post 
               ...
               target="_self"> 
<input type="hidden" name="idaxp_" value="<?php echo NM_encode_input($this->nmgp_dados_form['idaxp_']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo NM_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_archivosxpublicacion_inline']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo NM_encode_input($this->Ini->sc_page); ?>"/> 
  <input type="hidden" name="script_case_session" value="<?php echo NM_encode_input(session_id()); ?>"> 
</form> 
...
{
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_archivosxpublicacion']['where_detal']))
  {
?>  
  if (parent && parent.nmsc_iframe_liga_form_archivosxpublicacion.scInlineFormReceive)
  {
    parent.nmsc_iframe_liga_form_archivosxpublicacion.scInlineFormReceive(oResp, <?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['form_archivosxpublicacion_inline']['inline_form_seq'] ?>);
    return true;
  }
<?php
  }
...
?>  
  ...
  {
    parent.scInlineFormReceive(oResp, <?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['form_archivosxpublicacion_inline']['inline_form_seq'] ?>);
    return true;
  }
<?php
  }
...
function nm_mostra_doc(campo1, campo2, campo3)
{
    ...
    NovaJanela = window.open ("form_archivosxpublicacion_inline_doc.php?script_case_init=<?php echo NM_encode_input($this->Ini->sc_page); ?>&script_case_session=<?php echo session_id() ?>&nm_cod_doc=" + campo1 + "&nm_nome_doc=" + encodeURIComponent(campo2) + "&nm_cod_apl=" + campo3, "ScriptCase", "resizable, scrollbars");
}
