<?php
//--- 
class grid_log_actividades_det
{
   var $Ini;
   var $Erro;
   var $Db;
 ...
 function monta_det()
 {
    ...
    $_SESSION['sc_session'][$this->Ini->sc_page]['grid_log_actividades']['seq_dir'] = 0; 
    $_SESSION['sc_session'][$this->Ini->sc_page]['grid_log_actividades']['sub_dir'] = array(); 
   ...
   $this->Ini->sc_Include($this->Ini->path_lib_php . "/nm_edit.php", "F", "nmgp_Form_Num_Val") ; 
       $nmgp_select = "SELECT id_actividad, usuario, sesion, actividad, creado from " . $this->Ini->nm_tabela; 
   $parms_det = explode(";", $_SESSION['sc_session'][$this->Ini->sc_page]['grid_log_actividades']['chave_det']) ; 
   ...
   $nmgp_select .= " where  id_actividad = $parms_det[0]" ;  
   ...
   $this->id_actividad = $rs->fields[0] ;  
   $this->id_actividad = (string)$this->id_actividad;
   $this->usuario = $rs->fields[1] ;  
   $this->usuario = (string)$this->usuario;
   $this->sesion = $rs->fields[2] ;  
   $this->sesion = (string)$this->sesion;
   $this->actividad = $rs->fields[3] ;  
   $this->creado = $rs->fields[4] ;  
   if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_log_actividades']['cmp_acum']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_log_actividades']['cmp_acum']))
   {
       $parms_acum = explode(";", $_SESSION['sc_session'][$this->Ini->sc_page]['grid_log_actividades']['cmp_acum']);
...
//--- 
   ...
   $nm_saida->saida("<HEAD>\r\n");
   $nm_saida->saida("   <TITLE>" . $this->Ini->Nm_lang['lang_othr_detl_titl'] . " - log_actividades</TITLE>\r\n");
   ...
   if (($_SESSION['sc_session'][$this->Ini->sc_page]['grid_log_actividades']['det_print'] == "print" && strtoupper($nmgp_cor_print) == "PB") || $nmgp_tipo_pdf == "pb")
   {
   ...
   if (!$_SESSION['sc_session'][$this->Ini->sc_page]['grid_log_actividades']['pdf_det'] && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_log_actividades']['det_print'] != "print")
   {
   ...
   $nm_saida->saida("    	<tr>\r\n");
   $nm_saida->saida("        	<td id=\"lin1_col1\" class=\"scGridHeaderFont\"><span>" . $this->Ini->Nm_lang['lang_othr_detl_titl'] . " - log_actividades</span></td>\r\n");
   ...
   $nm_saida->saida("  </TD>\r\n");
   $nm_saida->saida(" </TR>\r\n");
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_log_actividades']['det_print'] != "print" && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_log_actividades']['pdf_det']) 
   { 
       $nm_saida->saida("   <tr><td class=\"scGridTabelaTd\">\r\n");
       ...
       {
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bpdf", "", "", "Dpdf_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "grid_log_actividades/grid_log_actividades_config_pdf.php?nm_opc=pdf_det&nm_target=0&nm_cor=cor&papel=1&orientacao=1&largura=1200&conf_larg=S&conf_fonte=10&language=es&conf_socor=S&KeepThis=false&TB_iframe=true&modal=true", "", "only_text", "text_right", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
       }
       if ($this->nmgp_botoes['det_print'] == "on")
       {
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bprint", "", "", "Dprint_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "grid_log_actividades/grid_log_actividades_config_print.php?nm_opc=detalhe&nm_cor=AM&language=es&KeepThis=true&TB_iframe=true&modal=true", "", "only_text", "text_right", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
       }
   ...
   $nm_saida->saida("  <TR class=\"scGridLabel\">\r\n");
          ...
          $conteudo = trim(NM_encode_input($this->id_actividad)); 
          if ($conteudo === "") 
          { 
              $conteudo = "&nbsp;" ; 
          } 
   ...
   $nm_saida->saida("  <TR class=\"scGridLabel\">\r\n");
          ...
          $conteudo = trim(NM_encode_input($this->sesion)); 
          if ($conteudo === "") 
          { 
              $conteudo = "&nbsp;" ; 
          } 
...
//--- 
   ...
   $nm_saida->saida("   {\r\n");
   $nm_saida->saida("       NovaJanela = window.open (\"grid_log_actividades_doc.php?script_case_init=" . NM_encode_input($this->Ini->sc_page) . "&script_case_session=" . session_id() . "&nm_cod_doc=\" + campo1 + \"&nm_nome_doc=\" + campo2 + \"&nm_cod_apl=\" + campo3, \"ScriptCase\", \"resizable\");\r\n");
   $nm_saida->saida("   }\r\n");
   $nm_saida->saida("   function nm_gp_move(x, y, z, p, g) \r\n");
   $nm_saida->saida("   {\r\n");
   $nm_saida->saida("      window.location = \"" . $this->Ini->path_link . "grid_log_actividades/index.php?nmgp_opcao=pdf_det&nmgp_tipo_pdf=\" + z + \"&nmgp_parms_pdf=\" + p +  \"&nmgp_graf_pdf=\" + g + \"&script_case_init=" . NM_encode_input($this->Ini->sc_page) . "&script_case_session=" . session_id() . "\";\r\n");
   $nm_saida->saida("   }\r\n");
   $nm_saida->saida("   function nm_gp_print_conf(tp, cor)\r\n");
   $nm_saida->saida("   {\r\n");
   $nm_saida->saida("       window.open('" . $this->Ini->path_link . "grid_log_actividades/grid_log_actividades_iframe_prt.php?path_botoes=" . $this->Ini->path_botoes . "&script_case_init=" . NM_encode_input($this->Ini->sc_page) . "&script_case_session=" . session_id() . "&opcao=det_print&cor_print=' + cor,'','location=no,menubar,resizable,scrollbars,status=no,toolbar');\r\n");
   $nm_saida->saida("   }\r\n");
